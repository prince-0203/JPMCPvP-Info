...
詳しくは「No License」で調べてください。一応下にQiitaの記事の引用を載せておきます。  
ただし、GitHubでソースコードを公開し、Pull Requestを受け入れているため、利用者は以下のことが許可されています。
...
- :x:Forkしたソースコードをウェブサーバー上で公開する。要はパクリサイト禁止。
