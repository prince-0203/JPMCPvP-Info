...
もともとソースコード公開の予定はなかったのでコメントも少ないですしコードも汚いです。

# リポジトリの運用について
- 一部のファイルは非公開です。
- IssueやPull Requestは自由に送ってくださって構いません。
- ソースコードに関する質問や要望もIssueに送ってくださって構いません。
- (以下自分のための備忘録)
- 小規模な変更(リンク切れ・Typoの修正等)はmasterへ直接コミットします。
- それ以外はブランチを作成してコミットします。

# ライセンス
特に明示しません。**これは、権利を放棄したということでも、自由に使用できるようにしたという意味ではありません。**  
但し、GitHubでソースコードを公開してる以上、利用者は以下のことが許可されています。
- 自由にソースコードを閲覧する。:o:
- ソースコードをForkする。:o:
- Forkしたソースコードを自分で研究するために改造する。:o:
  * この場合は、改造したものを公開することはできません。:x:
- ForkしたソースコードをPull Requestを送るために改造する。:o:
  * この場合は、改造したものを公開できます。:o:
...
┃┣ logo.png - ロゴにする予定。どうしてもヘッダーにうまく収まらない。  
┃┣ logo-white.png - 同上。  
